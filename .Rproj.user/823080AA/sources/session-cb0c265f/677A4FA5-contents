rm(list = ls())
pacman::p_load('here','readr','qqman','dplyr','RColorBrewer','ggplot2','gridGraphics','grid','ggplot2','gridExtra','tidyverse','egg')

dir <- c('oneDose','twoDose','severityCovid','susceptibilityCovid',
         'susceptibilityOmicron','susceptibilityPreOmicron')
...
for (ii in 1:length(dir)){
  gwas <- read_delim(here('GWAS','ImputedData','Results',paste0(dir[ii],'.txt'))) %>%
    mutate(P = 10^(-LOG10P))  %>%
    select(CHR = CHROM, BP = GENPOS, SNP = ID, LOG10P, P) %>%
    filter(!is.na(LOG10P))
  
  source(here("GWAS","getManhattanPlot.R"))
  plot1 <- getManhattanPlot(gwas,y_lim = ylim_MH[ii])
  source(here("GWAS","getQQPlot.R"))
  ...
  ggsave(here("GWAS","Figures",paste0('MH2_',dir[ii],'.png')), plot = p, width = 240, height = 140, dpi = 300, units = 'mm')
